...
<div class="card card-preview">
	<div class="card-inner mt-3">
		<form action="{{ route('panel.user.update', $user) }}" method="POST" enctype="multipart/form-data">
			@csrf
			<div class="row">
					...
					<div class="row">
						...
						<div class="mb-3 col-sm-12 col-lg-3">
							...
							<select name="status" class="form-control" id="status">
								<option @if($user->status == 'Active') selected @endif value="Active">{{ __('Active') }}</option>
								<option @if($user->status == 'Inactive') selected @endif value="Inactive">{{ __('Inactive') }}</option>
							</select>
						</div>
